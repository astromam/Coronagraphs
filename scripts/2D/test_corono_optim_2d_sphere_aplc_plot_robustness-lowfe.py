...
nopd   = 6
opd_nm_t = np.sqrt(10.)**(np.arange(nopd))
opd_t = opd_nm_t*1e-9
...
res = (mydist <= rho1bis)*(mydist >= rho0bis)
...
direct_poly_avg_aberr_t = np.zeros((nmodes,nopd))
corono_poly_avg_aberr_t = np.zeros((nmodes,nopd))
...
for imode in range(nmodes):
    for iopd in range(nopd):
        ...
        corono_poly_img_aberr_t[imode, iopd] = corono_poly_img_aberr_t[imode, iopd, res != 0]
        corono_poly_img_aberr_t[imode, iopd] /= direct_poly_img_f.max()
