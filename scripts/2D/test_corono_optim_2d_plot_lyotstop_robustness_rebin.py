...
# -*- coding: utf-8 -*-
"""
Created on Wed Aug 15 11:46:35 2018
...
zLyotStop2d_t = []
pix_t = np.arange(-zm, zm+1, 1)
nzcorono = len(list(pix_t))

for j in range(2):
    for i in range(nzcorono):
        if pix_t[i] == 0:
            zLyotStop2d_t.append(zLyotStop2d)
        else:
            zLyotStop2d_t.append(np.roll(zLyotStop2d, pix_t[i], axis=j))
...
#%% image plot
    ...
    fname = fname_gen + '_lyotstop_axis={0}_shift={1}_zm={2}.pdf'.format(k//nzcorono,pix_t[k%nzcorono],zm)
    fpath = fdir_pdf / fname
    
    pl.figure(10*k+6)
    pl.clf()
    pl.imshow(zcorono_t[k].LyotStop2d, cmap = 'Greys_r')
    pl.title('Lyot stop - axis={0} - shift={1} - zm={2}'.format(k//nzcorono,pix_t[k%nzcorono],zm))
    pl.tight_layout()
    if zm <= 4:
        pl.savefig(str(fpath), transparent=True)
    if k >= 5:
    ...
    fname = fname_gen + '_direct_image_axis={0}_shift={1}_zm={2}.pdf'.format(k//nzcorono,pix_t[k%nzcorono],zm)
    fpath = fdir_pdf / fname

    pl.figure(10*k)
    pl.clf()
    pl.imshow(np.log10(poly_direct_image_t[k]/poly_direct_image_t[k].max()), cmap = 'inferno', vmin=-8, vmax=-2.)
    pl.title('Apod1 - direct image - axis={0} - shift={1} - zm={2}'.format(k//nzcorono,pix_t[k%nzcorono],zm))
    cbar = pl.colorbar()
    ...
    fname = fname_gen + '_apodized_image_axis={0}_shift={1}_zm={2}.pdf'.format(k//nzcorono,pix_t[k%nzcorono],zm)
    fpath = fdir_pdf / fname
    
    pl.figure(10*k+1)
    pl.clf()
    pl.imshow(np.log10(poly_corono_image_t[k]/poly_direct_image_t[k].max()), cmap = 'inferno', vmin=-8, vmax=-2.)
    pl.title('Apod1 - apodized image - axis={0} - shift={1} - zm={2}'.format(k//nzcorono,pix_t[k%nzcorono],zm))
    cbar = pl.colorbar()
    cbar.set_label('Normalized intensity in log scale')
    pl.tight_layout()
    if zm <= 4:
...
#%% Intensity profiles of the images in broadband light
    ...
    fname = fname_gen + '_intensity_profiles_axis={0}_shift={1}_zm={2}.pdf'.format(k//nzcorono,pix_t[k%nzcorono],zm)
    fpath = fdir_pdf / fname
    
    pl.figure(10*k+2)
    pl.clf()
    pl.title('Radial intensity profiles of the images - axis={0} - shift={1} - zm={2}'.format(k//nzcorono,pix_t[k%nzcorono],zm))
    if corono_name == 'SP':
