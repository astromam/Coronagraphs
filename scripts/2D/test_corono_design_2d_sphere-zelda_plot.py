...
#pl.figure(32)
#pl.clf()
#pl.semilogy(rad_corono*Fmax2d/nImg2d*1000.*(wv/dAper)/mas2rad, corono_poly_prf_std_t[i0]/direct_poly_img_t[i0].max(),
#                '--', label='simulation - map #{0}/{1}'.format(i0, nmap-1), color = colors_map[0])
#
##pl.semilogy(cor_sep_sky, cor_zel_prf_sky_avg/psf_zel_norm_sky)
#if imap0 == 0:    
...
#pl.title(r'Intensity profile in broadband light ($\Delta\lambda/\lambda_0$={0:.1f}%)'.format(bw*100))
#pl.tight_layout()
#pl.savefig(str(fpath_image_allmaps_plot_comp), transparent=True)
#
#pl.show()
#
#
