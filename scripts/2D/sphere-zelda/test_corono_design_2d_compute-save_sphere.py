...
#%%
fdir_pupils  = Path('.').resolve() / 'data' / '2D' / 'pupils' / 'SPHERE' 
fdir_results = Path('.').resolve() / 'results' / '2D' / 'data' / 'SPHERE'/ 'tests'
...
params     = coro.update_params(params, OPDmap2d = None, Ampmap2d = None, LyotStop2d = LyotStop2dth)
corono00th = coro.design.APLC2d(**params)

params     = coro.update_params(params, OPDmap2d = None, Ampmap2d = None, LyotStop2d = LyotStop2d)
corono00   = coro.design.APLC2d(**params)

params     = coro.update_params(params, OPDmap2d = None, Ampmap2d = Ampmap2d, LyotStop2d = LyotStop2d)    
corono10   = coro.design.APLC2d(**params)

for imap, val in enumerate(OPDmapnm3d):
    t0 = time.time()
    print('computation for map {0}'.format(imap))
    OPDmap2d = val*1e-9    

    params   = coro.update_params(params, OPDmap2d = OPDmap2d, Ampmap2d = None, LyotStop2d = LyotStop2d) 
    corono01 = coro.design.APLC2d(**params)
    
    params   = coro.update_params(params, OPDmap2d = OPDmap2d, Ampmap2d = Ampmap2d, LyotStop2d = LyotStop2d)
    corono11 = coro.design.APLC2d(**params)
