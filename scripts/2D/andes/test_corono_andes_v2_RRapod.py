...
# Mask dize of the Roddier & Roddier for the ELT with nPup=400
...
# Directory for the Roddier & Roddier apodization
...
"""
# Read ELT pupil 
Pupil2d = fits.getdata(fpath_elt,)

# Read Roddier & Roddier apodizer for ELT pupil
...
"""
# Focal plane mask
mask2d = uniform_disk(nFPM, nFPM/2., CtrBtwnPix=True)

# Lyot stop
LyotStop2d = Pupil2d*1

#LyotStop2d = (uniform_disk(nPup, 0.95*nPup/2)-uniform_disk(nPup, 0.35*nPup/2))*Pupil2d
...
"""
# focal plane B 
Fld_BB = mask2d*sft(Fld_AA0, nFPM, mB, CtrBtwnPix=True)

# pupil plane C before Lyot stop
Fld_CC = Fld_AA0 - (1 - np.exp(1j*np.pi))*isft(Fld_BB, nPup, mB, CtrBtwnPix=True)
...
#for iPSF in range(len(fpath_psf)):
    ...
    for iOPD in range(nOPD):
        # pupil plane A
        Fld_A0 = Apod2d * Pupil2d * np.exp(1j*2*np.pi*OPD_arr[iOPD]/lam)
        
        # focal plane B 
        Fld_B = mask2d*sft(Fld_A0, nFPM, mB, CtrBtwnPix=True)
        
        # pupil plane C before Lyot stop
        Fld_C = Fld_A0 - (1- np.exp(1j*np.pi))*isft(Fld_B, nPup, mB, CtrBtwnPix=True)
    ...
    """
    # filename for the direct and coronagraphic images
    fname_Int_D0 = 'seed=' + str(seed) + '_psf_RRapod.fits'
    fname_Int_D = 'seed=' + str(seed) + '_cor_RRapod.fits'
    ...
    fname_images = 'seed=' + str(seed) + '_images_RRapod.pdf'
    ...
    im = grid[2].imshow(np.log10(Int_DD), vmin=vmin0, vmax=vmax0, cmap='inferno')
    grid[2].set_title('Perfect coro. image')
    
    # AO corrected coronagraphic image
    im = grid[3].imshow(np.log10(Int_D), vmin=vmin0, vmax=vmax0, cmap='inferno')
    ...
    """
    # filename of the plot
    fname_prf = 'seed=' + str(seed) + '_profiles_RRapod.pdf'
