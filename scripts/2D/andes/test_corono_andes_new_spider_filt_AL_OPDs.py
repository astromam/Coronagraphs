...
# new_spider = fdir_dat / 'PASSATA/20240228_112033.0_Seeing_0_65/' 
...
# fpath_opd = fpath_opd[200:201]  #5min temps exposition
...
# Read OPD maps for the nOPD files
# OPD_arr = np.asarray([fits.getdata(fpath_opd[i]) for i in range(1)])
# OPD_arr = OPD_arr*1e-9 # convert OPD from nm `to m if new OPD with new pupil
#**
OPD_filtered_AL = np.load('/Users/asimonnin/Desktop/PhD/Andes/results/OPD_Adrien/OPD_filtré_AL6.npy')*1e-9
...
for k in range(2): 
    ...
    if k ==0 : 
        ...
        fname_Int_D0 = 'Filtered_OPDs_AL6.fits'
        fname_Int_D = 'Filtered_OPDs_coro_AL6.fits'
        ...
        for iOPD in range(2000):
            ...
            Fld_A0 = Pupil * np.exp(1j*2*np.pi*data_windshake[iOPD]/(lam)) * LyotStop2d ## Avec Lyot stop
        ...
        for iOPD in range(2000):
            # pupil plane A
            Fld_A0 = Pupil * np.exp(1j*2*np.pi*data_windshake[iOPD]/(lam))
        ...
        # fname_Int_D0 = 'seed=' + str(seed) + 'new_spider_psf_flare_'+str(i)+'.fits'
        # fname_Int_D = 'seed=' + str(seed) + 'new_spider_cor_flare_'+str(i)+'.fits'
        fname_Int_D0 = 'No_Filtered_OPDs_AL7.fits'
        fname_Int_D = 'No_Filtered_OPDs_coro_AL7.fits'
...
fname_Int_D0_no_filtered = 'No_Filtered_OPDs_AL6.fits'
fname_Int_D_no_filtered = 'No_Filtered_OPDs_coro_AL6.fits'

fname_Int_D0_filtered = 'Filtered_OPDs_AL6.fits'
fname_Int_D_filtered = 'Filtered_OPDs_coro_AL6.fits'
