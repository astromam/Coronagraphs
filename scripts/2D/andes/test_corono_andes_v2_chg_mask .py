...
# lyot_config = '3'
    ...
    # plt.imshow(LyotStop2d)
        ...
        for s in range(len(mB_conf)) :
            ...
            fdir_plt = fdir_plt / 'lyot_diam_{diam}_obst_{obst}_mB={mB}_bande_J'.format(diam=diam,obst = obst,mB=mB)
...
# np.save(fdir_res / fname_fract_lum, fraction_lum)
            ...
            for iPSF in range(len(fpath_psf)):
                ...
                for iOPD in range(nOPD):
                    ...
                    # Field in the image plane D (no coronagraph)
                    Fld_D0 = sft(Fld_C0, nImg, mD*diam)#diametre_lyot
                ...
                for iOPD in range(nOPD):
                    ...
                    # image plane D 
                    Fld_D = sft(Fld_L, nImg, mD*diam) #diametre_lyot
