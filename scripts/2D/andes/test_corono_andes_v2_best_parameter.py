...
if wl == 'H' : 
    ...
    lam_list = np.arange(1.4e-6,1.84e-6,0.04e-6)
    # print(lam_list)
    # pdb.set_trace()
...
for lam in lam_list :
    ...
    diam = 0.9
    obst = 0.4
    mB = mB_0 * lam_0/lam # mutiplier par lambda_0/lamdba
    ...
    fdir_plt = fdir_plt / 'lyot_diam_{diam}_obst_{obst}_mB={mB}_bande_{band}_wl_{lam:.2f}'.format(diam=diam,obst=obst,mB=mB,band=wl,lam=lam*1e6)
    fdir_res = fdir_res / 'lyot_diam_{diam}_obst_{obst}_mB={mB}_bande_{band}_wl_{lam:.2f}'.format(diam=diam,obst=obst,mB=mB,band=wl,lam=lam*1e6)
    ...
    # plt.colorbar()
    # plt.savefig(fdir_plt / 'lyot_effect_lam_{lam}.png'.format(lam=lam), dpi=300)
    # plt.show()
    # pdb.set_trace()
    ...
    for i in range (1):
        ...
        for iOPD in range(nOPD):
            # Field in the entrance pupil plane A
            Fld_A0 = Pupil_coro * np.exp(2*1j*np.pi*2*OPD_arr[iOPD]/lam) 
            # print(lam,lam_0)
            # Field in the entrance pupil plane C
        ...
        fname_Int_D0 = 'seed=' + str(seed) + '_psf_'+coro_config+'_2OPD.fits'
        fname_Int_D = 'seed=' + str(seed) + '_cor_'+coro_config+'_2OPD.fits'
