...
# -*- coding: utf-8 -*-
"""
Created on Thu Jan 15 17:53:33 2026
...
#    fdir = Path('/Users/mndiaye/OneDrive - Université Nice Sophia Antipolis/data/Coronagraphs/data/2D/pupils/').resolve()
    ...
    fdir_pupils = Path('/Users/mndiaye/scratch/data/Coronagraphs/data/2D/pupils/SPHERE/')
    
    fpath_lys = fdir_pupils / 'sphere_stop_ST_ALC2.fits'  # fdir_dat / fname_lys
    Pupil2d0    = aperture.vlt_pupil(nPup, nPup, dead_actuator_diameter=0) #fits.getdata(fpath_pup)
...
#%%
"""
### projected pupil on a tilted apodizer
"""
plt.figure(0)
plt.clf()
plt.imshow(Pupil2d)
...
xi = (2/nPup)*(np.arange(nPup)-nPup/2+1/2)
yi = xi*1.
...
# Pupil2d_x = np.zeros((nPup, nPup))
# Pupil2d_y = np.zeros((nPup, nPup))
# Pupil2d_x = xx*Pupil2d
# Pupil2d_y = yy*Pupil2d

# plt.figure(3)
# plt.clf()
# plt.subplot(121)
# plt.imshow(Pupil2d_x)
# plt.subplot(122)
# plt.imshow(Pupil2d_y)


#%%
xibis = xi*np.cos(delta)
yibis = yi*1.

xxbis, yybis = np.meshgrid(xibis, yibis)

rrbis = np.sqrt(xxbis**2 + yybis**2)
...
NewOuter2d = np.zeros((nPup, nPup))
NewOuter2d[rrbis <= 1.] = 1.

ID = 1100./8000.
NewInner2d = np.zeros((nPup, nPup))
NewInner2d[rrbis <= ID] = 1.

NewPupil2d = NewOuter2d - NewInner2d

plt.figure(4)
plt.clf()
plt.imshow(NewPupil2d)
...
spdrdelta = int(spdr/np.cos(delta))
    
ref = np.zeros((tdim, tdim))
ref[cc:, cc:cc+spdrdelta] = 1
spider1 = _rotate_interp(ref, -5.5, (cc, (cc+diameter/2)))

ref = np.zeros((tdim, tdim))
ref[:cc, cc-spdrdelta+1:cc+1] = 1
spider2 = _rotate_interp(ref, -5.5, (cc, (cc-diameter/2)))

ref = np.zeros((tdim, tdim))
ref[cc:cc+spdrdelta, cc:] = 1
spider3 = _rotate_interp(ref, 5.5, (cc+diameter/2, cc))

ref = np.zeros((tdim, tdim))
ref[cc-spdrdelta+1:cc+1, :cc] = 1
spider4 = _rotate_interp(ref, 5.5, (cc-diameter/2, cc))

#spider0 = _rotate_interp(spider1+spider2+spider3+spider4, 45+spiders_orientation, (cc, cc), delta)
...
#%%
NewPupil2dwithSpiders = NewPupil2d*spider0


plt.figure(5)
plt.clf()
plt.imshow(NewPupil2dwithSpiders)
...
plt.figure(50)
plt.clf()
plt.imshow(OPDmap2d_init)

aTilt = 0.*wv #1000*wv*(1*np.pi/180)/(2.*np.pi)
OPDmap2d_init *= aTilt
...
if corono_name == 'APLC':
    poly_direct_image1 = corono0.compute_direct_intensity_2d_bis(Apod2d, OPDmap2d = None)
else:
    poly_direct_image1 = corono0.compute_direct_intensity_2d(corono0.Pupil2d, OPDmap2d = OPDmap2d_init)
poly_corono_image1 = corono0.compute_corono_intensity_2d_bis(Apod2d, OPDmap2d = OPDmap2d_init)
...
"""
mono_direct_image1 = corono0.compute_direct_intensity_2d_bis(Apod2d, OPDmap2d = None, poly=False)
mono_corono_image1 = corono0.compute_corono_intensity_2d_bis(Apod2d, OPDmap2d = OPDmap2d_init, poly=False)
