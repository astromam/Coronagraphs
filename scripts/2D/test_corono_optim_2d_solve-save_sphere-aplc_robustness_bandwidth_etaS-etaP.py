...
pl.clf()
pl.imshow(Apod_pyth*corono00.Pupil2d, cmap = 'inferno')
...
if corono_name == 'APLC':
    ...
    direct_mono_img_tN = coronoN.compute_direct_intensity_2d(Apod_pyth, poly=False)
elif corono_name == 'SP':
...
for l in range(nsep):
    ...
    corono_mono_img_t3_P[l]  = corono33.compute_corono_intensity_2d(Apod_pyth, poly = False)
t1 = time.time()
...
test = eta_S_arr/eta_P_arr

#%%
pl.figure(31)
pl.clf()
for i in range(nlam_ter-1):
    pl.plot(sep_arr, np.log10(test[:, i]/test[:, i].max())),
            color = colors_wv[i])
pl.show()
...
#%%
fname_image_plane_f_disp = 'corono_poly_bw_sensitivity_contour_nPup={0}_etaS-etaP_disp.pdf'.format(nPup)
fpath_image_plane_f_disp = fdir_plots / fname_image_plane_f_disp
