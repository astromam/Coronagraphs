...
# -*- coding: utf-8 -*-
"""
Created on Mon Jul 28 16:26:28 2025
...
if True:
    ...
    pupil_name   = 'elt-andes' #'vlt_btw' # 'vlt' or 'sbr' or 'lvr'
...
#    fdir = Path('/Users/mndiaye/OneDrive - Université Nice Sophia Antipolis/data/Coronagraphs/data/2D/pupils/').resolve()
    ...
    if pupil_name == 'elt-andes':
        fpath_pup = fdir_dat / pupil_name / fname_pup
        fpath_lys = fdir_dat / pupil_name / fname_lys
...
fname     = fname_gen + f'{str_dead_act}' + f'{str_ConstrApodImg}' + str_LSRcoeff_pre + str_LSRcoeff_bis + str_LSRcoeff_qua + str_LSRcoeff_qua2 + str_LSRcoeff_sev + str_LSRcoeff_v8 + str_LSRcoeff_v9 + '.fits'
...
fig = plt.figure(11)
plt.clf()
im = plt.imshow(np.log10(poly_corono_image1), vmin = -8, vmax = 0, cmap = cm.inferno)
...
if corono_name == 'SP':
    plt.semilogy(xi2d,poly_corono_image1[nImg2dbis//2,nImg2dbis//2:]/poly_corono_image1.max(),label='broadband')
else:
    plt.semilogy(xi2d,poly_corono_image1[nImg2dbis//2,nImg2dbis//2:]/poly_direct_image1.max(),label='broadband') 
...
#%% Intensity profiles of the direct and coronagraphic images
"""
### Display of the intensity profiles of the direct images in broadband light
"""
xi2d = corono0.xi2d
if nImg2dbis%2 == 0:
    xi2d = corono0.xi2d_ctr
...
nImg2d = corono0.params['nImg2d']
fname = fname_gen + '_intensity_profiles_broadband_direct.pdf'
fpath = fdir_pdf / fname

plt.figure(22)
plt.clf()
plt.title('Radial intensity profiles of the PSFs')

if corono_name == 'SP':
    plt.semilogy(xi2d,poly_corono_image1[nImg2dbis//2,nImg2dbis//2:]/poly_corono_image1.max(),label='broadband')
else:
    plt.semilogy(xi2d,poly_direct_image1[nImg2dbis//2,nImg2dbis//2:]/poly_direct_image1.max(),label='broadband') 
    
if test_shift:
    if corono_name == 'SP':
        plt.semilogy(xi2d,poly_corono_image1_shift[ishift_all0, nImg2dbis//2,nImg2dbis//2:]/poly_corono_image1.max(),label=solver+ ' shift', ls=':')
    else:
        plt.semilogy(xi2d,poly_direct_image1_shift[ishift_all0, nImg2dbis//2,nImg2dbis//2:]/poly_direct_image1.max(),label=solver +' shift', ls=':') 
    
plt.axvline(x=corono0.rMask, ymin=-12, ymax =2, linewidth=1, color='r', linestyle='--')
plt.axvline(x=corono0.rho0, ymin=-12, ymax =2, linewidth=1, color='b', linestyle='--')
plt.axvline(x=corono0.rho1, ymin=-12, ymax =2, linewidth=1, color='b', linestyle='--')
plt.axhline(10**(-cDarkHole_nocoro), xmin=corono0.xi.min(), xmax=corono0.xi.max(), linewidth=1, color='k', linestyle='--')
plt.xlabel(r'Angular separation in $\lambda_0$/D')
plt.ylabel('Normalized intensity in log scale')
plt.ylim(1e-6, 2e0)
plt.legend()
...
#%%
"""
### Display of the intensity profiles of the direct images in monochromatic light
"""
values = range(nlambis)
colors = plt.cm.rainbow(np.linspace(0,1,nlambis))
...
for ilam in range(corono0.nlam):
    if corono_name == 'SP':
        ...
        plt.semilogy(xi2d,mono_direct_image1[ilam, nImg2dbis//2,nImg2dbis//2:], 
                    '-',
                    label=r'{0:.2f}$\lambda_0$'.format(corono0.lam_t[ilam]),  
                    color = colors[ilam])
...
plt.semilogy(r_lamD,poly_corono_prf_avg,label='broadband')
...
#%%
"""
### Display of the averaged intensity profiles of the direct images in broadband light
"""
xi2d = corono0.xi2d
if nImg2dbis%2 == 0:
    xi2d = corono0.xi2d_ctr
...
nImg2d = corono0.params['nImg2d']
fname = fname_gen + '_intensity_profiles_broadband_avg_direct.pdf'
fpath = fdir_pdf / fname
...
plt.semilogy(r_lamD,poly_direct_prf_avg,label='broadband')

if test_shift:
    plt.semilogy(r_lamD,poly_direct_shift_prf_avg[ishift_all0],label=solver + ' shift', ls=':')
...
nImg2d = corono0.params['nImg2d']
fname = fname_gen + '_intensity_profiles_monochromatic_avg_direct.pdf'
fpath = fdir_pdf / fname
...
for ilam in range(corono0.nlam):
    plt.semilogy(r_lamD, mono_direct_prf_avg[ilam],
                 label=r'{0:.2f}$\lambda_0$'.format(corono0.lam_t[ilam]),  color = colors[ilam])
