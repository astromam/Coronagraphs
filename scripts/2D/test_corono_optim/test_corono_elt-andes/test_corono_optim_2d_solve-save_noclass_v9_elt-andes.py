...
# -*- coding: utf-8 -*-
"""
Created on Mon Jul 28 16:14:30 2025
...
pupil_name   = 'elt-andes' # 'vlt' or 'sbr' or 'lvr' or 'vlt_btw'
...
#nlam
shift_tot0 = (np.sqrt(2.)/0.5)*int(0.005*nPup0) # (np.sqrt(2.)/0.5)*int(0.005*nPup0) #np.sqrt(shift_x**2+shift_y**2)
...
str_ConstrApodImg = ''
if ConstrApodImg:
    str_ConstrApodImg = '_ConstrApodImg'
...
def compute_direct_field_2d_qrt(Apod2d_qrt, Pupil2d_qrt, LyotStop2d_qrt, corono=None):
    """
    Computes the coronagraph electric field for a classical Lyot coronagraph
    with four planes (A: entrance pupil, B: intermediate focal plane, 
    C: relayed pupil before stop, L: relayed pupil after stop, 
    ...
    if corono is None:
        ...
        for i in range(corono.nlam):                       
            field_L_qrt       = field_A_qrt*LyotStop2d_qrt
            field_Dtmp_qrt[i] = sft_qrt(field_L_qrt, corono.nImg2d, corono.mD_t[i], 
                      CtrBtwnPix=True)
...
#%%    
def compute_direct_field_2d_LSasym(Apod2d_qrt, Pupil2d_qrt, LyotStop2d, corono=None):
    """
    Computes the coronagraph electric field for a classical Lyot coronagraph
    with four planes (A: entrance pupil, B: intermediate focal plane, 
    C: relayed pupil before stop, L: relayed pupil after stop, 
    ...
    if corono is None:
        ...
        for i in range(corono.nlam):                                   
            field_C           = np.zeros((corono.nPup, corono.nPup))
            field_C[corono.nPup//2:, corono.nPup//2:] = field_A_qrt
            field_C[:corono.nPup//2, corono.nPup//2:] = np.flip(field_A_qrt, axis=0)
            field_C[:, :corono.nPup//2]               = np.flip(field_C[:, corono.nPup//2:], axis=1)
...
#%%
def compute_direct_matrices_qrt(idx_pup, idx_dz, npp, ndz,corono=None):
    r"""
    Computes the response matrix for the coronagraph with and without 
    the focal plane mask.
    ...
    if corono is None:
        ...
        if ImPart is True:
            direct_field_re_t_tmp = np.empty((npp, corono.nlam, 
                                                   corono.nImg2d**2))             
            direct_field_im_t_tmp = np.empty((npp, corono.nlam, 
                                               corono.nImg2d**2))

            for i, val in enumerate(idx_pup):
                (i0,j0) = np.unravel_index(val, (corono.nPup//2, corono.nPup//2))
                Apod2d_qrt[i0,j0] = 1            
                test = (1./4)*compute_direct_field_2d_LSasym(Apod2d_qrt, Pupil2d_qrt, corono.LyotStop2d, corono)
                direct_field_re_t_tmp[i] = np.reshape(test.real, (nlam, nImg2d**2))
                direct_field_im_t_tmp[i] = np.reshape(test.imag, (nlam, nImg2d**2))
                Apod2d_qrt[i0,j0] = 0 
                
            direct_field_re_t = np.reshape(
                    direct_field_re_t_tmp[:,:, idx_dz], 
                    (npp, corono.nlam*ndz))
            
            direct_field_re_t_tmp = None
            del direct_field_re_t_tmp
        
            direct_field_im_t = np.reshape(
                direct_field_im_t_tmp[:,:, idx_dz], 
                (npp, corono.nlam*ndz))

            direct_field_im_t_tmp = None
            del direct_field_im_t_tmp
            
            return direct_field_re_t, direct_field_im_t
            ...
            for i, val in enumerate(idx_pup):
                (i0,j0) = np.unravel_index(val, (corono.nPup//2, corono.nPup//2))
                Apod2d_qrt[i0,j0] = 1
                test_qrt = (1./4)*compute_direct_field_2d_qrt(Apod2d_qrt, Pupil2d_qrt, LyotStop2d_qrt, corono)
                direct_field_re_t_tmp[i] = \
                np.reshape(test_qrt, (corono.nlam, (corono.nImg2d//2)**2))
                # corono.compute_corono_field_2d_real_vec(Apod2d)
                Apod2d_qrt[i0,j0] = 0 
...
def compute_corono_field_2d_LSasym_LSrobustness_bis(Apod2d_qrt, Pupil2d_qrt, LyotStop2d, dist2d, str_axis, corono=None):
    ...
    if corono is None:
        ...
        # field_Dtmp = np.zeros((corono.nlam,corono.nImg2d,corono.nImg2d), dtype=dtype0)
        field_Dtmp_shift_grad_xy = np.zeros((corono.nlam,corono.nImg2d,corono.nImg2d), dtype='complex128')
        ...
        if ImPart:                    
            ...
            return np.real(field_Dtmp_shift_grad_xy)
    ...
    if corono is None:
        ...
        if ImPart:
            ...
            for i, val in enumerate(idx_pup):
                ...
                test = 0
                test = (1./4)*compute_corono_field_2d_LSasym_LSrobustness_bis(Apod2d_qrt, Pupil2d_qrt, corono.LyotStop2d, dist2d, str_axis, corono)
                corono_field_re_t_tmp[i] = np.reshape(test.real, (nlam, nImg2d**2)) #np.reshape(test_qrt, (corono.nlam, (corono.nImg2d//2)**2))
                # corono.compute_corono_field_2d_real_vec(Apod2d)
                Apod2d_qrt[i0,j0] = 0 
...
for k in range(nProgRef):
    ...
    elif pupil_name == 'elt-andes':
        fname_pup = 'ELT_pupil_400_sym.fits'
        fname_lys = 'ELT_pupil_400_sym.fits'
    else:
        raise NameError(f'{pupil_name}: unknown pupil name')
    
    fpath_pup = fdir / fname_pup
    fpath_lys = fdir / fname_lys
    if pupil_name == 'elt-andes':
        fpath_pup = fdir / pupil_name / fname_pup
        fpath_lys = fdir / pupil_name / fname_lys
...
#    if kwd_qrt:
    if ImPart:
        ...
        if k != 0:
            ...
            if ConstrApodImg:
                PsiDnocoro0 = (1./4)*compute_direct_field_2d_LSasym(Ones_2d2_qrt, Pupil2d2_qrt, LyotStop2d, corono0)
                PsiDnocoro0 = np.reshape(PsiDnocoro0, (corono0.nlam, corono0.nImg2d**2))
        ...
        if k != 0:
            ...
            if ConstrApodImg:
                PsiDnocoro0 = (1./4)*compute_direct_field_2d_qrt(Ones_2d2_qrt, Pupil2d2_qrt, LyotStop2d2_qrt, corono0)
                PsiDnocoro0 = np.reshape(PsiDnocoro0, (corono0.nlam, (corono0.nImg2d//2)**2)) 

    if LSRobustness:
        
        print('LS constraints')
        PsiD_grad_x_re = np.empty_like(PsiD_re) 
        PsiD_grad_x_im = np.empty_like(PsiD_im) 
        PsiD_grad_y_re = np.empty_like(PsiD_re) 
        PsiD_grad_y_im = np.empty_like(PsiD_im) 
        if ImPart:
    ...
    if problem_name == 'MaxContrastLinf' or problem_name == 'MaxContrastL1':
        ...
        if LSRobustness:
            if ImPart:
                ...
                model.addConstr(  PsiD_grad_x_re.T @ Apo + PsiD0bis.real - Eps/(shift_tot/corono0.nPup) <= 0)
                model.addConstr( -PsiD_grad_x_re.T @ Apo - PsiD0bis.real - Eps/(shift_tot/corono0.nPup) <= 0)
    
                model.addConstr(  PsiD_grad_y_re.T @ Apo + PsiD0bis.real - Eps/(shift_tot/corono0.nPup) <= 0)
                model.addConstr( -PsiD_grad_y_re.T @ Apo - PsiD0bis.real - Eps/(shift_tot/corono0.nPup) <= 0)
        ...
        if LSRobustness:
            if ImPart:
                ...
                model.addConstr(  PsiD_grad_x_re.T @ Apo - Psi0/(shift_tot/corono0.nPup) <= 0)
                model.addConstr( -PsiD_grad_x_re.T @ Apo - Psi0/(shift_tot/corono0.nPup) <= 0)   
    
                model.addConstr(  PsiD_grad_y_re.T @ Apo - Psi0/(shift_tot/corono0.nPup) <= 0)
                model.addConstr( -PsiD_grad_y_re.T @ Apo - Psi0/(shift_tot/corono0.nPup) <= 0)              
...
#    if kwd_qrt:
    ...
    fname_sav = get_filename(corono0) + f'{str_dead_act}' + f'{str_ConstrApodImg}'  + str_LSRcoeff_v9 + '.fits'
