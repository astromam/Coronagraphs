...
t0 = time.time()
if kwd_qrt:
    A[0:npp,0:nPsiD] = compute_response_matrices_qrt(npp,corono0) 
else:
    A[0:npp,0:nPsiD] = compute_response_matrices(corono0) 
t1= time.time()
...
A[npp:npp+neps,2*nPsiD+ndz] = 0

# Compute constraints on the apodizer transmission
A[0:npp,2*nPsiD+ndz+1:2*nPsiD+ndz+1+npp] = -np.identity(npp) #ok
A[npp:npp+neps,2*nPsiD+ndz+1:2*nPsiD+ndz+1+npp] = np.zeros((neps, npp)) #ok

A[:,2*nPsiD+ndz+1+npp:2*nPsiD+ndz+1+npp+npp] = -A[:,2*nPsiD+ndz+1:2*nPsiD+ndz+1+npp]
t11=time.time()
print(f'matrix A computation time: {t11-t00:.3f}s\n')

#%%
"""
### Computation of the matrix b
"""
#print('computing matrix b')
t00=time.time() 
b = np.zeros((nPsiD*2+ndz+1+npp*2))

b[:2*nPsiD] = 0
b[2*nPsiD:2*nPsiD+ndz] = 0
b[2*nPsiD+ndz:2*nPsiD+ndz+1] = -tau

b[2*nPsiD+ndz+1:2*nPsiD+ndz+1+npp] = 0 
b[2*nPsiD+ndz+1+npp:2*nPsiD+ndz+1+npp*2] = 1
t11=time.time()
#print(f'matrix b computation time: {t11-t00:.3f}s\n')
...
c[npp:npp+neps] = c1
t11=time.time()
#print(f'matrix c computation time: {t11-t00:.3f}s\n')

#%%
"""
### Dimensions
"""
print(f'dimensions A: {np.shape(A)}')
print(f'A axis-0: {np.shape(A)[0]}')
print(f'A axis-1: {np.shape(A)[1]}')
print(f'dimensions b: {np.shape(b)}')
print(f'dimensions c: {np.shape(c)}')
print(f'nlam: {nlam}')
print(f'ndz: {ndz}')
