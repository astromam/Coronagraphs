...
# -*- coding: utf-8 -*-
"""
Created on Thu Dec  5 17:26:38 2024
...
def compute_corono_field_2d_LSasym_LSrobustness_bis(Apod2d_qrt, Pupil2d_qrt, LyotStop2d, dist2d, shift_tot, alpha, corono=None):
    ...
    if corono is None:
        ...
        return np.abs(field_Dtmp_shift_ana-field_Dtmp)


#%%
def compute_response_matrices_qrt_LSrobustness_bis(idx_pup, idx_dz, npp, ndz, shift_tot, alpha_t, corono=None):
    r"""
    Computes the response matrix for the coronagraph with and without 
    the focal plane mask.
    ...
    if corono is None:
        ...
        if ImPart is True:
            ...
            for i, val in enumerate(idx_pup):
                ...
                Apod2d_qrt[i0,j0] = 1
                test = 0
                for ialpha, alpha in enumerate(alpha_t):
...
#    if kwd_qrt:
    ...
    fname_sav = get_filename(corono0) + f'{str_dead_act}' + str_LSRcoeff_v8 + '.fits'
