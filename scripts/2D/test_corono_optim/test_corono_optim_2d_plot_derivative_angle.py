...
# -*- coding: utf-8 -*-
"""
Created on Mon Dec  2 16:37:57 2024
...
if True:
    ...
    alpha = np.pi/4 # np.pi/3 # np.arctan2(shift_x, shift_y)
...
if do_FPM: 
    ...
    for ialpha, alpha0 in enumerate(alpha_t):
    
        # dot product term insde the complex exponential to represent the shift in spatial domain
        xidr = (2.*np.pi)*(yyp*(shift_tot*np.cos(alpha0)/nPup) + xxp*(shift_tot*np.sin(alpha0)/nPup))
        ...
        for i in range(corono0.nlam):    
            ...
            field_Dtmp_shift_ana[i] += np.abs(coro.utils.sft(field_L_shift_ana, corono0.nImg2d, corono0.mD_t[i]*(nPup/nPupLS), 
                      CtrBtwnPix=True)) 
                
        field_Dtmp_shift_ana /= nalpha
...
#%%
ilam0 = corono0.nlam//2

#%%
plt.figure(10, (12, 4.5))
plt.clf()
plt.subplot(131)
...
#    var1 = [shift_tot, alpha]
    var1 = [shift_y, shift_x]
    
    t0 = time.time()
    grad1 = nd.Gradient(fct_field_Dtmp_mono_shift_dif)(var1)
    t1 = time.time() 
    
    print(f'time: {t1-t0:.2f}s')
    
    #%%
    plt.figure(20, (16, 4.5))
    plt.clf()
    plt.subplot(141)
    plt.imshow(np.log10(np.abs((field_Dtmp_shift_ana[corono0.nlam//2]-field_Dtmp[corono0.nlam//2]))**2), cmap='inferno', vmin=vmin0, vmax=vmax0)
    plt.title(r'$|\Psi_{Dana}^{dr}-\Psi_D|^2$')
    plt.subplot(142)
    plt.imshow(np.log10(np.abs(grad1[:,0])), cmap='inferno', vmin=vmin0, vmax=vmax0)
    plt.title(r'$|\nabla_{x}\Psi_D|$')
    plt.subplot(143)
    plt.imshow(np.log10(np.abs(grad1[:,1])), cmap='inferno', vmin=vmin0, vmax=vmax0)
    plt.title(r'$|\nabla_{y}\Psi_D|$')
    plt.subplot(144)
    plt.imshow(np.log10(np.sqrt(np.abs(grad1[:,0])**2+np.abs(grad1[:,1])**2)), cmap='inferno', vmin=vmin0, vmax=vmax0)
    plt.title(r'$\sqrt{|\nabla_{y}\Psi_D|^2+|\nabla_{y}\Psi_D|^2}$')
