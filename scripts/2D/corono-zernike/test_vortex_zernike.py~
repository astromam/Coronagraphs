...
"""
fld_ZZref2_tmp = fld_LLwfe - (1.-eps_R2)\
               *coro.utils.isft(fld_YYwfe, nPup_big, mB_big, CtrBtwnPix=CtrBtwnPix)

int_ZZref2_tmp = np.abs(fld_ZZref2_tmp)**2  

int_ZZref2 = int_ZZref2_tmp[np.int(nPup*((k_big-1)/2)):np.int(nPup*((k_big-1)/2))+nPup,
                            np.int(nPup*((k_big-1)/2)):np.int(nPup*((k_big-1)/2))+nPup]*\
                            LyotStop2d
...
int_LLref     = int_LLref_tmp[np.int(nPup*((k_big-1)/2)):np.int(nPup*((k_big-1)/2))+nPup,
                            np.int(nPup*((k_big-1)/2)):np.int(nPup*((k_big-1)/2))+nPup]
...
#%%
fname_pdf = 'ZWFS_images_before-after_ZWFSmask_ref=RR.pdf'
fpath_pdf = fdir_pdf / fname_pdf
...
fname_pdf = 'ZWFS_reconstruction_ref=00.pdf'
fpath_pdf = fdir_pdf / fname_pdf
...
fname_pdf = 'ZWFS_reconstruction_ref=RR.pdf'
fpath_pdf = fdir_pdf / fname_pdf
