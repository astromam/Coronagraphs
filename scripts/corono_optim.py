...
t1 = time.time()
print('solver time             : {0:.2f}s'.format(t1-t0))
...
#Export apodizer fits file
fdir = 'apodizers/{0}/'.format(pupil_name)
if not os.path.exists(fdir):
    os.makedirs(fdir)

fname = problem1.get_filename() + '_{0:02d}D{1:02d}_telserv3.fits'.format(int(round(100*inD)), int(round(100*outD)))
fpath = fdir + fname
...
#Apodizer
fname = fname_gen + '_{0:02d}D{1:02d}_apodisation_telserv3.pdf'.format(int(round(100*inD)), int(round(100*outD)))
fpath = fdir_pdf + fname
pl.figure(7)
pl.clf()
...
#Direct image
fname = fname_gen + '_direct_image_telserv3.pdf'
fpath = fdir_pdf + fname
pl.figure(8)
pl.clf()
...
#Write direct image to fits
fname = fname_gen + '_apodized_image_telserv3.fits'
fpath = fdir_pdf + 'fits_images/' + fname
...
fname = fname_gen + '_broadband_intensity_profiles_telserv3.pdf'
fpath = fdir_pdf + fname
...
fname = fname_gen + '_monochrom_intensity_profiles_telserv3.pdf'
fpath = fdir_pdf + fname
