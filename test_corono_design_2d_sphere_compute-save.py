...
direct_poly_img_t = np.zeros((ncase, nImg2d, nImg2d))
corono_poly_img_t = np.zeros((ncase, nImg2d, nImg2d))
direct_mono_img_t = np.zeros((ncase, nlam, nImg2d, nImg2d))
corono_mono_img_t = np.zeros((ncase, nlam, nImg2d, nImg2d))

for icase in range(ncase):
    direct_poly_img_t[icase] = coro_lst[icase].compute_direct_intensity_2d(Apod2d)
    corono_poly_img_t[icase] = coro_lst[icase].compute_corono_intensity_2d(Apod2d)
    direct_mono_img_t[icase] = coro_lst[icase].compute_direct_intensity_2d(Apod2d, poly=False)
    corono_mono_img_t[icase] = coro_lst[icase].compute_corono_intensity_2d(Apod2d, poly=False)

direct_poly_prf_avg_t = np.zeros((ncase, nImg2d//2))
corono_poly_prf_avg_t = np.zeros((ncase, nImg2d//2))
direct_mono_prf_avg_t = np.zeros((ncase, corono00.nlam, nImg2d//2))
corono_mono_prf_avg_t = np.zeros((ncase, corono00.nlam, nImg2d//2))

direct_poly_prf_std_t = np.zeros((ncase, nImg2d//2))
corono_poly_prf_std_t = np.zeros((ncase, nImg2d//2))
direct_mono_prf_std_t = np.zeros((ncase, corono00.nlam, nImg2d//2))
corono_mono_prf_std_t = np.zeros((ncase, corono00.nlam, nImg2d//2))

for icase in range(ncase):
    direct_poly_prf_avg_t[icase], rad_direct = imutils.profile(direct_poly_img_t[icase], type='mean')
    corono_poly_prf_avg_t[icase], rad_corono = imutils.profile(corono_poly_img_t[icase], type='mean')
    direct_poly_prf_std_t[icase], rad_direct = imutils.profile(direct_poly_img_t[icase], type='std')
    corono_poly_prf_std_t[icase], rad_corono = imutils.profile(corono_poly_img_t[icase], type='std')
    for i in range(corono00.nlam):
        direct_mono_prf_avg_t[icase, i], rad_direct = imutils.profile(direct_mono_img_t[icase, i], type='mean')
        corono_mono_prf_avg_t[icase, i], rad_corono = imutils.profile(corono_mono_img_t[icase, i], type='mean')
        direct_mono_prf_std_t[icase, i], rad_direct = imutils.profile(direct_mono_img_t[icase, i], type='std')
        corono_mono_prf_std_t[icase, i], rad_corono = imutils.profile(corono_mono_img_t[icase, i], type='std')
...
#%%
direct_mono_lyot_t = np.zeros((ncase, nlam, nPup, nPup), dtype='complex128')
corono_mono_lyot_t = np.zeros((ncase, nlam, nPup, nPup), dtype='complex128')
for icase in range(ncase):
    direct_mono_lyot_t[icase] = coro_lst[icase].compute_direct_lyot_field_2d(Apod2d)
    corono_mono_lyot_t[icase] = coro_lst[icase].compute_corono_lyot_field_2d(Apod2d)
