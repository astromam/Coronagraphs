...
fpath_cyth = fdir_pyth / pupil_name / fname_gen.format(**{key: corono0.params[key] for key in corono0.params})

print('{0}'.format(fpath_cyth))

Apod_cyth = fits.getdata(fpath_cyth,)
...
pl.figure(6)
pl.clf()
pl.imshow(Apod_cyth*corono0.Pupil2d, cmap = cm.Greys_r)
pl.title('Apod 1 transmission - cyth')
pl.tight_layout()
pl.savefig(str(fpath))
...
if corono_name == 'APLC':
    ...
    poly_direct_image2 = corono0.compute_direct_intensity_2d(Apod_cyth)
...
poly_corono_image2 = corono0.compute_corono_intensity_2d(Apod_cyth)
...
pl.title('Apod1 - apodized image - cyth')
pl.tight_layout()
pl.savefig(str(fpath))
...
if corono_name == 'SP':
    pl.semilogy(corono0.xi2d,poly_corono_image1[nImg2d//2,nImg2d//2:]/poly_corono_image1.max(),label='Apod - pyth')
    pl.semilogy(corono0.xi2d,poly_corono_image2[nImg2d//2,nImg2d//2:]/poly_corono_image2.max(),label='Apod - cyth')
else:
