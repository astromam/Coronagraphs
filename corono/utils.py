...
#%%
def newsft(A2, NB, m, inv=False, CtrBtwnPixIn=False, CtrBtwnPixOut=False):
    """
    Slow Fourier Transform, using the theory described in [1]_. 
    Assumes the original array is square. 
    ...
    valA    = 0
    valB    = 0
    if CtrBtwnPixIn is True:
        valA = 1/2
    if CtrBtwnPixOut is True:
        valB = 1/2
    NA    = np.shape(A2)[0]
    coeff = m/(NA*NB)
    ...
    X[0,:] = (1./NA)*(np.arange(NA)-NA/2.+valA)
    U[0,:] =  (m/NB)*(np.arange(NB)-NB/2.+valB)
...
#%%
def newisft(A2, NB, m, CtrBtwnPixIn=False, CtrBtwnPixOut=False):
    """
    Explicit inverse Slow Fourier Transform, using the theory described in [1].
    ...
    """
    return newsft(A2, NB, m, inv=True, CtrBtwnPixIn=CtrBtwnPixIn, CtrBtwnPixOut=CtrBtwnPixOut)
